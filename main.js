...
(function () {
    ...
    var updateTime = '2020年4月5日15:24:12';
    ...
    if (window.console && window.console.log) {
        console.clear();
        console.log("%c 当前版本：Hidove名华在线老实挂机版 %c https://blog.hidove.cn ", "color: #fadfa3; background: #030307; padding:5px 0;", "background: #fadfa3; padding:5px 0;");
                    ...
                    if (layIndex === null) {
                        ...
                        getBase64('/service/code?r=' + Math.random())
                            .then(function (base64) {
                                $.ajax({
                                    'url': 'https://api.hidove.cn/api/Captcha/distinguish',
                                    'type': 'post',
                                    'data': {
                                        token: "5d8f31cf6a8ab",
                                        img: base64,
    ...
    //获取当前课程所有id数组
    function getIdList() {
        let data = storage.get('idList_' + courseId);
        arr = JSON.parse(data);
        if (arr != null) {
            return arr;
        }
        var list = document.querySelectorAll(".two .list .item a")
        var arr = [];
        ...
        storage.set('idList_' + courseId, JSON.stringify(arr));
        return arr;
    }
    //获取当前Id
